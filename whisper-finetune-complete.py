...
class WhisperFineTuner:
    def __init__(
        ...
        self.training_args = Seq2SeqTrainingArguments(
            ...
            fp16=mixed_precision == "fp16" and self.device == "cuda",  # Only use fp16 if CUDA is available
    ...
    def prepare_dataset(self, csv_path: str, split_name: str = "train") -> Dataset:
        ...
        processed_dataset = dataset.map(
            ...
            num_proc=4 if self.device == "cuda" else 1  # Adjust number of processes based on device
        )
...
def main():
    # Initialize fine-tuner with device-aware configuration
    fine_tuner = WhisperFineTuner(
        ...
        per_device_train_batch_size=16 if torch.cuda.is_available() else 4,  # Reduce batch size for CPU
        per_device_eval_batch_size=8 if torch.cuda.is_available() else 2,    # Reduce batch size for CPU
    )
