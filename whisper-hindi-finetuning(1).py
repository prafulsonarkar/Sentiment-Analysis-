...
class AudioDataProcessor:
    def __init__(
        ...
        self.augmentations = {
            'noise': torchaudio.transforms.AddNoise(snr=20.0),
            'time_mask': torchaudio.transforms.TimeMasking(time_mask_param=50),
            'pitch_shift': torchaudio.transforms.PitchShift(sampling_rate=sample_rate, n_steps=4)
        }
            ...
            try:
                ...
                if augment:
                    for aug_name, augmentation in self.augmentations.items():
                        if np.random.rand() < self.aug_probs[aug_name]:
                            try:
                                waveform = augmentation(waveform)
                            except Exception as e:
                                logger.warning(f"Augmentation {aug_name} failed: {str(e)}")
